...
public static class IslandFinder
{
    ...
    public static void DFS(bool[,] matrix, int x, int y, bool[,] visited, bool check = true)
    {
        ...
        for (int k = 0; k < 8; ++k)
            if (IsSafe(matrix, x + rowNbr[k], y + colNbr[k], visited, check))
                DFS(matrix, x + rowNbr[k],y + colNbr[k], visited, check);
    }

    /// Returns count of islands in a given boolean 2D matrix
    public static int CountIslands(bool[,] matrix, bool check=true)
    {
